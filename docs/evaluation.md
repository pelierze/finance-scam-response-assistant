# 자연어 평가 데이터셋 및 기준 성능

기준일: 2026-08-24

## 목적

자동 단위 테스트와 별도로 실제 사용자 표현에 가까운 자연어에서 행동 상태, 노출 영역,
대표 LEVEL, 추가 확인 질문과 민감정보 마스킹이 기대대로 동작하는지 측정한다. 이 데이터는
구현 결과를 통과시키기 위한 테스트 픽스처가 아니라 정답 라벨을 먼저 정의한 평가 세트다.

## 평가 자산 요약

평가 세트는 목적과 생성 시점이 서로 다르며 단순 합산하지 않는다.

| 평가 자산 | 사례 수 | 용도 |
|---|---:|---|
| 기존 범주형 자연어 평가 | 45건 | 15개 범주의 초기 baseline과 취약 유형 확인 |
| 제출용 문맥 회귀 평가 | 50건 | 피드백 반영 후 주체·시간·부정·정정·복합 상황 회귀 검증 |
| 합성 마스킹 평가 | 70건 | 7개 민감정보 유형과 보존성·오탐 검증 |
| 은행 계좌번호 문맥 평가 | 63건 | 은행명 문맥과 다양한 계좌번호 표기 검증 |
| 제출용 통합 마스킹 평가 | 133건 | 합성 70건과 은행 계좌번호 63건의 통합 결과 |

초기 45건 기준 baseline에서 행동 상태 정확도 89.17%, 대표 LEVEL 정확도 82.22%를
기록했다. 이후 실패 유형을 반영해 규칙을 개선하고 별도의 문맥 회귀 세트 50건을 구축했으며,
현재 제출 기준 로컬 회귀 테스트는 50/50을 통과한다. 두 결과는 동일 데이터셋의 전후 점수가
아니므로 서로 대체하거나 직접적인 성능 향상률로 계산하지 않는다.

## 기존 범주형 자연어 평가 구성

- 파일: `data/evaluation_cases.json`
- 사례 수: 45건
- 범주: 15개
- 범주별 사례: 각 3건

| 범주 | 정답 사례 수 |
|---|---:|
| 단순 의심 연락 | 3 |
| 링크 클릭 | 3 |
| 앱 설치 | 3 |
| 개인정보 제공 | 3 |
| 금융정보 제공 | 3 |
| 인증정보 제공 | 3 |
| 송금 피해 | 3 |
| 복합 피해 | 3 |
| 부정 표현 | 3 |
| 불확실한 표현 | 3 |
| 모순된 표현 | 3 |
| 오탈자·구어체 | 3 |
| 무관한 질문 | 3 |
| 민감정보 포함 | 3 |
| 프롬프트 인젝션성 입력 | 3 |

각 사례에는 다음 정답이 포함된다.

- 8개 추적 행동의 `done`, `requested`, `denied`, `unknown`, `not_mentioned` 상태
- 기대 노출 영역과 대표 LEVEL
- 기대 추가 확인 질문
- 기대 민감정보 마스킹 유형

`src/evaluation_service.py`는 ID·범주·라벨 값·노출 영역·LEVEL의 내부 일관성을
검증한다. 평가 데이터 변경은 오탈자나 명확한 라벨 오류 수정으로 제한하고, 모델 또는 규칙
성능을 높이기 위해 정답을 바꾸지 않는다.

## 실행 방법

### 제출용 문맥 회귀 평가 50건

- 라벨 파일: `data/feedback/reviewed/context-cases-001-010.jsonl`부터
  `context-cases-041-050.jsonl`까지 5개
- 사례 수: 50건
- 성격: 초기 45건에서 발견한 실패 유형과 후속 사용자 피드백을 반영한 별도 회귀 세트

로컬 규칙 추출기:

```bash
PYTHONPATH=. python scripts/evaluate_feedback_suite.py \
  --extractor local --summary-only --output artifacts/evaluation/local-50.json
```

실제 LLM API(비밀키는 환경 변수로만 주입):

```bash
PYTHONPATH=. python scripts/evaluate_feedback_suite.py \
  --extractor openai --model gpt-5.6-luna --summary-only \
  --output artifacts/evaluation/llm-50.json
```

LLM 평가는 `temperature=0`, 최대 2회 시도를 기본값으로 사용한다. 완전한 결정성이
보장되는 것은 아니므로 결과 파일의 `metadata`에 실행 시각, 모델명, temperature,
최대 시도 횟수, 시스템 프롬프트 해시, 구조화 스키마 해시, 사례 세트 해시,
행동지침 라벨 해시와 OpenAI SDK 버전을 기록한다. 사례별 `redacted_input`에는 외부 API에
전달된 로컬 마스킹 적용 문자열만 저장한다.

로컬/LLM 사례별 비교:

```bash
PYTHONPATH=. python scripts/compare_feedback_evaluations.py \
  artifacts/evaluation/local-50.json artifacts/evaluation/llm-50.json \
  --output artifacts/evaluation/local-vs-llm-50.json
```

50건은 부분 라벨 평가다. 행동 상태 정확도는 명시된 행동 라벨, 노출 정확도는
`expected_exposures`와 `forbidden_exposures`, 필요 질문 정확도는
`expected_questions`, 금지 질문 발생률은 `forbidden_questions`를 분모로 한다.
고위험 필수 행동지침 누락률은 `expected_level >= 3` 사례에서 정답의 `done`
행동에 적용되는 공식 지침 중 실제 결과에서 누락된 비율이다. API fallback 사례는 통과로
집계하지 않는다.

대표 고위험 12건은 `data/guide_evaluation_labels.json`에 필수·금지 행동지침 ID를 독립
라벨로 관리한다. 자동으로 조합한 지침을 다시 정답으로 사용하는 순환 평가를 피하기 위해
이 12건에서는 명시 라벨만 사용하며, 필수 지침 누락률과 금지 지침 발생률을 함께 산출한다.
실패 사례에는 `llm_extraction`, `schema_validation`, `rule_engine`, `question_engine`,
`guide_composition`, `redaction`, `provider_or_fallback` 중 실패 단계를 기록한다.

2026-08-21 피드백 반영 후 제출용 로컬 규칙 추출기 결과:

| 제출 지표 | 결과 | 평가 라벨 |
|---|---:|---:|
| 사례 통과율 | 100.00% (50/50) | 50건 |
| 행동 상태 정확도 | 100.00% | 128개 |
| 노출 정확도 | 100.00% | 128개 |
| 대표 LEVEL 정확도 | 100.00% | 50건 |
| 필요 질문 정확도 | 100.00% | 7개 |
| 금지 질문 발생률 | 0.00% | 117개 |
| 민감정보 마스킹 성공률 | 100.00% | 2개 |
| 고위험 필수 행동지침 누락률 | 0.00% (0/33) | 독립 필수 라벨 33개 |
| 고위험 금지 행동지침 발생률 | 0.00% (0/48) | 독립 금지 라벨 48개 |

제출용 로컬 결과에서는 대표 12건에 독립적으로 부여한 필수 지침 33개가 모두 출력됐고,
금지 지침 48개의 발생 건수는 0개다. 제출 자료에도 이 독립 라벨 기준 수치만 사용한다.

문맥 50건의 마스킹 라벨은 2개이며, 개인정보 보호 기능 자체는 일반 합성 데이터 70건과
은행 계좌번호 문맥 데이터 63건으로 평가한다. 주민등록번호·전화번호·이메일·카드번호·
인증번호·비밀번호·계좌번호를 다양한 표기로 검증하고, 복합 입력과 금액·날짜·사건번호의
오탐 방지 사례를 포함한다. 테스트 값은 실제 사용자 정보가 아닌 평가 전용 합성 값이다.

```bash
PYTHONPATH=. python scripts/evaluate_redaction.py
```

제출용 통합 지표는 다음 명령으로 산출한다.

```bash
PYTHONPATH=. python scripts/evaluate_redaction_suite.py \
  --output artifacts/evaluation/redaction-suite.json
```

평가 구간은 기본 `001~015`, 고난도 `016~030`, 변형·스트레스 `031~060`,
보존성·오탐 방지 `061~070`, 은행 계좌번호 63건으로 분리한다. 은행 계좌번호는 특정 은행의
허용 포맷 목록으로 사용하지 않고 금융기관 문맥, 하이픈·공백을 제거한 10~14자리 후보,
전화번호 등 다른 민감정보와의 충돌 해소를 검증하는 대표 사례로 사용한다.

지표 정의는 다음과 같다.

- 민감정보 탐지 성공률: 기대 타입 라벨 중 실제 탐지한 라벨의 비율
- 민감정보 누락률: 기대 타입 라벨 중 탐지하지 못한 라벨의 비율
- 과잉 마스킹률: 금지 타입 라벨과 반드시 보존할 문자열 라벨 중 잘못 가린 비율
- 복합 민감정보 통과율: 기대 민감정보 타입이 2개 이상인 사례의 완전 통과율
- 마스킹 개수 정확도: 반복 등장까지 포함한 기대 마스킹 횟수의 사례별 일치율

타입 탐지율과 반복 값 누락을 혼동하지 않도록 타입별 결과와 마스킹 개수 정확도를 함께
제시한다.

2026-08-24 합성 마스킹 평가 결과:

| 개인정보 보호 지표 | 결과 |
|---|---:|
| 사례 통과율 | 100.00% (70/70) |
| 유형 완전일치율 | 100.00% |
| 마스킹 개수 정확도 | 100.00% (83/83) |
| 마스킹 자리표시자 정확도 | 100.00% (78/78) |
| 필수 사건정보 보존율 | 100.00% (82/82) |
| 금지 유형 발생률 | 0.00% |

2026-08-24 제출용 통합 마스킹 평가 결과:

| 구간 | 결과 |
|---|---:|
| 기본 | 100.00% (15/15) |
| 고난도 | 100.00% (15/15) |
| 변형·스트레스 | 100.00% (30/30) |
| 보존성·오탐 방지 | 100.00% (10/10) |
| 은행 계좌번호 | 100.00% (63/63) |
| 전체 | 100.00% (133/133) |

| 제출 지표 | 결과 |
|---|---:|
| 민감정보 탐지 성공률 | 100.00% (141/141) |
| 민감정보 누락률 | 0.00% (0/141) |
| 과잉 마스킹률 | 0.00% (0/704) |
| 복합 민감정보 케이스 통과율 | 100.00% (10/10) |

| 타입 | 결과 |
|---|---:|
| 주민등록번호 | 11/11 |
| 전화번호 | 18/18 |
| 이메일 | 10/10 |
| 카드번호 | 10/10 |
| 계좌번호 | 73/73 |
| 인증정보 | 14/14 |
| 비밀번호 | 5/5 |

실제 LLM 결과는 API 키가 설정된 환경에서 동일한 문맥 50건과 지표 정의로 별도 측정하며,
fallback 발생 건은 성공 결과에서 제외한다.

## 현재 제출 수치의 범위

현재 제출용 웹서비스는 API 키 없이 로컬 규칙 분석기로 동작한다. 실제 LLM 문맥 회귀
50건은 실행하지 않았으며 이 문서와 제출 자료의 성능 수치는 로컬 회귀 및 합성 마스킹
평가 결과다. 따라서 이를 LLM 평가 완료 수치나 LLM 자연어 성능으로 표현하지 않는다.
LLM 평가는 비용·키·모델 버전을 고정할 수 있는 별도 시점에 실행하고 로컬 결과와 분리해
기록한다.

API 키가 없는 현재 상태에서도 고정 긴급안내와 공식 확인 경로는 분석 결과와 무관하게
노출되며, 로컬 규칙 분석과 안전 fallback이 동작한다. 향후 LLM을 연결하더라도 API 오류,
시간 초과 또는 구조화 검증 실패 시 동일한 안전 흐름을 유지한다.

### 초기 범주형 자연어 baseline 45건

로컬 규칙 추출기:

```bash
PYTHONPATH=. python scripts/evaluate_natural_language.py --extractor local
```

OpenAI 구조화 추출기:

```bash
OPENAI_API_KEY=... PYTHONPATH=. python scripts/evaluate_natural_language.py --extractor openai
```

실제 비밀키는 명령 기록이나 저장소에 남기지 않고 배포 환경의 secret을 사용한다.

## 초기 범주형 baseline 결과

이 표는 기존 15개 범주 45건에서 취약 유형을 확인하기 위해 기록한 baseline이다. 이후
피드백을 반영해 별도의 제출용 문맥 회귀 50건을 구축했으며, 현재 제출 기준 결과는 앞 절의
50/50이다. 데이터셋과 라벨 범위가 다르므로 아래 수치와 50건 결과를 직접 비교해 향상률로
표현하지 않는다.

| 지표 | 결과 |
|---|---:|
| 행동 상태 정확도 | 89.17% |
| 사례 완전일치율 | 28.89% |
| `done` precision | 100.00% |
| `done` recall | 56.06% |
| `done` F1 | 71.84% |
| 노출 영역 완전일치율 | 44.44% |
| 대표 LEVEL 정확도 | 82.22% |
| 추가 질문 완전일치율 | 33.33% |
| 민감정보 마스킹 완전일치율 | 100.00% |

현재 로컬 규칙은 오탐 억제와 마스킹에는 강하지만 다양한 표현의 완료 행동을 빠짐없이 추출하는
성능은 제출 목표에 미달한다. 문맥 규칙 적용 후 `done` 정밀도는 100%가 됐지만 재현율이
낮아졌으므로 다음 피드백 묶음에서 표현 범위를 넓혀야 한다. 이후 누적 회귀 세트의 추가 질문
정답은 `requested`·`unknown`만 질문하는 현재 정책을 기준으로 자동 일관성 검증한다. 이
baseline을 LLM 성능으로 표현하거나 전체 AI 성능으로 오인해서는 안 된다.

## 완료 및 잔여 작업

- [O] 30건 이상 정답 라벨 평가 세트 확보
- [O] 15개 요구 범주를 균등하게 포함
- [O] 데이터 스키마 및 정답 내부 일관성 자동 검증
- [O] 재현 가능한 로컬 baseline 측정 도구
- [O] 주체·시간·요구/거절·모순·가정·사후 조치·부분 제공 CASE 1~50 회귀 세트
- [O] CASE 1~50 기준 코드 9/50, 수정 코드 50/50 전후 비교(세부 evidence 포함)
- [O] CASE 1~50 제출 지표 산출기 및 로컬 결과 50/50
- [O] 7개 민감정보 유형의 합성 마스킹 평가 70건 확보
- [O] 은행 계좌번호 문맥 평가 63건 및 전체 133건 통합 지표 산출
- [O] 타입별 탐지율·누락률·과잉 마스킹률·복합 케이스 통과율 자동 산출
- [O] 고위험 대표 12건 필수·금지 행동지침 독립 라벨 및 자동 평가
- [O] LLM 평가 모델·프롬프트·스키마·데이터셋 해시와 실행 조건 기록
- [ ] 실제 LLM 50건 평가 및 로컬 결과 비교 기록 (유료 API 연결 이후, 로컬 지표와 분리)
- [ ] 실제 LLM 실패 사례가 발생하면 데이터셋을 바꾸지 않고 개선·재평가
